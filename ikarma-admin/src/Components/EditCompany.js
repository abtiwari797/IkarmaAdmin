...
const EditCompany = () => {
  ...
  useEffect(() => {
    ...
    const country = countries.find(country => Number(country.id) === company.countryid);

    const state = states.find(state => Number(state.id) === company.stateid);

    const city = cities.find(city => Number(city.id) === company.cityid);
    ...
    toast.info(
      `Country: ${country?.name}, State: ${state?.name}, City: ${city?.name}`,
      {
        ...
        autoClose: false, // Make it permanent
  ...
  return (
    <>
      <ToastContainer
        position="top-right"
        autoClose={false} // This ensures that the toast does not close automatically
        hideProgressBar={true} // Optionally hide the progress bar
        closeButton={false} // Optionally disable the close button
      />
      <Form
      ...
      >
        ...
        <Form.Item
          ...
          rules={[
            {
              required: true,
              message: "Please select if you are an ikarma partner",
            },
          ]}
        >
          <Radio.Group
